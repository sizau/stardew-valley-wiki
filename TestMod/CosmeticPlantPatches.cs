...
namespace TestMod
{
    ...
    internal class CosmeticPlantPatches
    {
        ...
        internal static bool performToolAction_Prefix(CosmeticPlant __instance, ref bool __result, Tool t, int explosion, Vector2 tileLocation)
        {
            try
            {
                ...
                if ((t is MeleeWeapon weapon && weapon.type.Value != 2) || explosion > 0)
                {
                    ...
                    Reflection.GetMethod(__instance, "shake").Invoke((float)Math.PI * 3f / 32f, (float)Math.PI / 40f, Game1.random.NextBool());
                    ...
                    Game1.createRadialDebris(location, __instance.textureName(), new Rectangle(__instance.grassType.Value * 16, 6, 7, 6), (int)tileLocation.X, (int)tileLocation.Y, Game1.random.Next(6, 14));
            ...
            {
                Monitor.Log($"在补丁 {nameof(performToolAction_Prefix)} 中发生错误:\n{ex}", LogLevel.Error);
                return true;
            }
        }
    }
